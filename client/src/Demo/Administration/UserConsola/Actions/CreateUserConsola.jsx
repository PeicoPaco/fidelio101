...
function CreateUserConsola() {
    ...
    const [staffFidelio,setFidelioStaff]=useState([]);
    const [emailFidelio,setEmailFidelio]=useState('');
    ...
    const toggleApiError = () => {
      if (typeof window.stackBottomLeft === 'undefined') {
        window.stackBottomLeft = {
            ...
            PNotify.error({
                title: 'Error',
                text: "No se pudo crear al miembro de Fidelio :(",
                stack: window.stackBottomLeft,
                delay:3000
            });
    };

    const postUserConsola = async (fidelio,token) => {

      const newUserConsola = {
        email: fidelio.email,
        profile: fidelio.profile,
        giveAccessAt: new Date(),
        giveAccessBy: email,
      }
            await fetch(`${process.env.REACT_APP_BASE_URL}/staffFidelio/`+fidelio.dni, {
            method: 'PUT',
      headers: {
      ...
      mode: 'cors',
      body: JSON.stringify(newUserConsola),
        }).then(() =>  toggleShow())
          .then(()=>sendPasswordResetEmail(auth, fidelio.email)
        ...
        useEffect(()=>{
          fetch(`${process.env.REACT_APP_BASE_URL}/staffFidelioByNoProfile`, {
            headers:{
              "Content-Type": "application/json",   
              Authorization: 'Bearer ' + reduxtoken,
            }
    ...
    const handleSubmit = (e) => {
        let dni = "";
        Object.values(staffFidelio).map((item) => {
          if(item.name === e.target.formName.value){
            dni = item.dni
          }
        });
        e.preventDefault();
        const emailFidelioStaff = e.target.formEmail.value;
        ...
        if (profilesCheckedArray.length === 0) {
          ...
          const fidelio = {
            email:emailFidelioStaff,
            dni:dni,
            profile:profilesCheckedArray,
          }
          postUserConsola(fidelio,reduxtoken);
        }
      }
    ...
    const handleName = (e) => {
      ...
      const namesWithOutProfile = staffFidelio.map((staff) => {
        return (
        ...
        <>
           <Row>
                <Col>
                    <Card className='m-3'>
                        <Card.Header>
                            <Card.Title as="h5">Crear Nuevo User Consola</Card.Title>
                        </Card.Header>
                        <Card.Body>
                            <Row>
                                <Col>
                                    <Form onSubmit={handleSubmit} className='justify-content'>
                                        ...
                                        <Form.Group controlId="formName" onChange={handleName} className=''>
                                            ...
                                            <Form.Control size="md" as="select" className='mr-3' placeholder='Email' required>
                                        ...
                                        <Form.Group controlId="formEmail" className=''>
                                            ...
                                            <Form.Control type='email' placeholder="Email" value={emailFidelio} disabled/>
                                        </Form.Group>
...
export default CreateUserConsola