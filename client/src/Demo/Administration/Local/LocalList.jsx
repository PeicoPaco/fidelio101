...
function atable(mydata, token) {  
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <p>¿Está seguro que desea eliminar el local ${row.name}?</p>
                                </div>
  ...
  $(tableZero).on('click','button.delete-btn',function(){
    ...
    const toggleShow = () => {
        ...
        PNotify.success({
            title: 'Eliminado',
            text: "El local se eliminó satisfactoriamente :D",
    ...
    const toggleApiError = () => {
        ...
        PNotify.error({
            title: 'Error',
            text: "No se logró eliminar el local :(",
            stack: window.stackBottomLeft,
            delay:3000
        });
};
    ...
    $.ajax({
        url: `${process.env.REACT_APP_BASE_URL}/local/` + data.id,
        type: 'DELETE',
        headers: {
  ...
  $(tableZero).on('click','button.edit-btn',function(){
    const data = $(tableZero).DataTable().row($(this).parents('tr')).data();
    window.location.href = '/editlocal/'+ data.id;
  });

  $(tableZero).on('click','button.details-btn',function(){
    const data = $(tableZero).DataTable().row($(this).parents('tr')).data();
    window.location.href = '/detailsLocal/'+ data.id;
  });
}
...
function LocalList() {
  ...
  useEffect(()=> {
            ...
            fetch(`${process.env.REACT_APP_BASE_URL}/local`, {
            headers:{
  ...
  return (<>
    <Row>
        <Col>
            <Card>
                <Card.Header>
                    <Card.Title as="h5">Lista de Locales</Card.Title>
                    {profile=="User"? 
                        null
                        :
                        (<NavLink to="/createlocal"><Button size='md'>+ Agregar</Button></NavLink>)}
                </Card.Header>
                <Card.Body>
